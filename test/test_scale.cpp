...
#define CHECK_TRIAD(rootDegree, s0, s1, s2)                         \
  do {                                                              \
    CHECK(scaleDegreeToSemitone((rootDegree) + 0) == (s0));         \
    CHECK(scaleDegreeToSemitone((rootDegree) + 2) == (s1));         \
    CHECK(scaleDegreeToSemitone((rootDegree) + 4) == (s2));         \
  } while (0)
...
static void runTests() {
  ...
  CHECK(scaleDegreeToSemitone(7) == 12);
  CHECK(scaleDegreeToSemitone(-1) == -1);  // the B below the key root
  CHECK(scaleDegreeToSemitone(-7) == -12);
  CHECK(scaleDegreeToSemitone(-8) == -13);

  // Snapping the pot's continuous note onto the key.
  CHECK(snapMidiToScaleDegree(60.0f) == 0);   // C4, the key root
  CHECK(snapMidiToScaleDegree(62.0f) == 1);   // D4
  CHECK(snapMidiToScaleDegree(60.9f) == 0);   // between C and D, nearer C
  CHECK(snapMidiToScaleDegree(61.4f) == 1);   // ... nearer D
  CHECK(snapMidiToScaleDegree(69.0f) == 5);   // A4
  CHECK(snapMidiToScaleDegree(72.0f) == 7);   // C5, an octave up
  CHECK(snapMidiToScaleDegree(59.0f) == -1);  // B3, below the key root
  ...
  CHECK(snapMidiToScaleDegree(66.0f) == 3);   // F#4 -> F4

  // Degrees resolve to real frequencies (C4 = 261.63 Hz, A4 = 440 Hz).
  CHECK_NEAR(scaleDegreeToFreq(0), 261.626f, 0.01f);
  CHECK_NEAR(scaleDegreeToFreq(5), 440.0f, 0.01f);
  CHECK_NEAR(scaleDegreeToFreq(7), 523.251f, 0.01f);
  ...
  CHECK(snapMidiToScaleDegree(freqToMidi(scaleDegreeToFreq(3))) == 3);
}
