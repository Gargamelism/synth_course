...
static int16_t hardClip(int16_t sample) {
  return clampToInt16((int32_t)sample * DISTORTION_DRIVE_GAIN);
}

// Cubic soft-clip: y = x - x^3/3, evaluated entirely in Q15 fixed point
// (VOLUME_Q15_SHIFT, the same fixed-point format the mixer uses) so it
// stays integer-only on the FPU-less C3. The curve is monotonic and
// self-bounded for |x| <= 32767, so no final clamp is needed.
static int16_t softClip(int16_t sample) {
...
// pathological cases.
static int16_t foldback(int16_t sample) {
...
static int16_t bitcrush(int16_t sample) {
...
int16_t applyDistortion(int16_t sample, DistortionType type) {
  switch (type) {
    case DIST_HARD_CLIP: return hardClip(sample);
    case DIST_SOFT_CLIP: return softClip(sample);
    case DIST_FOLDBACK:  return foldback(sample);
    case DIST_BITCRUSH:  return bitcrush(sample);
    case DIST_NONE:
