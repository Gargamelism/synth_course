...
- **Audio out**: `ESP_I2S.h` (`I2SClass`) → external PCM5102 I2S DAC. Each voice reads a composite wavetable (the harmonic sum precomputed per spread per octave in `oscillator.cpp`), so a voice costs one lookup per sample instead of a 22-term harmonic loop.
